...
export function HowItWorks({ className, ...rest }: Props) {
  return (
    <section className={`text-neutral-600 py-12 ${className ?? ""}`} {...rest}>
                ...
                <div className="hidden xl:flex flex-col basis-[340px] shrink-0 grow">
                  <ProjectFeature className="hidden xl:flex" noGrow>
                    <div>
                      ...
                      <span className="text-base">
                        Um lance é um valor que a pessoa está disposta a pagar por seu produto, pode ser maior ou menor
                        ao valor original.
                      </span>
                    </div>
                    <SnippetContainer>
                  ...
                  <ProjectFeature className="hidden xl:flex" noGrow>
                    <div>
                      ...
                      <span className="text-base">
                        Agora que seu post possui vários lances, você pode escolher quem você deseja premiar como
                        vencedor do leilão.
                      </span>
                    </div>
                    <SnippetContainer>
...
export function SnippetContainer({ children, fontColor, width, className, ...rest }: ISnippetContainer) {
  ...
  return (
    <div
      className={`p-3 md:p-6 flex flex-col my-3 font-roboto grow text-black whitespace-nowrap rounded-xl border border-dashed border-neutral-500 ${clsnm}`}
      {...rest}
    >
...
export function ProjectFeature({ children, className, noGrow, ...rest }: IProjectFeature) {
  return (
    <li
      className={`pt-4 flex flex-col md:border-none 
      ${className ?? ""}
      ${noGrow ? "" : "grow"}
      `}
      {...rest}
