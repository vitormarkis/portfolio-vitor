...
export const Logo = React.forwardRef<HTMLAnchorElement, ILogo>(({ className, scale = 1.2, style, ...rest }, ref) => {
  const codesize = scale * 18
  const fontSize = scale * 16
  const gap = scale * 6

  return (
    <Link href="/" ref={ref} className="p-5 rounded-2xl w-fit block mx-auto mdx:mx-0">
      <div
      ...
      >
        <i>
          <Code weight="bold" color={twc.violet["700"]} width={codesize} height={codesize} />
        </i>
        <span className={`font-bold ml-1 ${jetbrains.className}`} style={{ fontSize }}>
